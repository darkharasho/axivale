...
describe('MemoryPanel', () => {
  ...
  it('calls memoryPin when pin button is clicked', async () => {
    const pin = vi.fn().mockResolvedValue(undefined)
    ;(window as unknown as { officer: unknown }).officer = makeOfficer({ memoryPin: pin })
    render(<MemoryPanel />)
    await screen.findByText('Scourge is meta in WvW.')
  ...
  it('calls memoryDelete with "fact" when delete button is clicked', async () => {
    const del = vi.fn().mockResolvedValue(undefined)
    ;(window as unknown as { officer: unknown }).officer = makeOfficer({ memoryDelete: del })
    render(<MemoryPanel />)
    await screen.findByText('Scourge is meta in WvW.')
  ...
  it('calls memoryUpdate to archive a fact', async () => {
    ...
    ;(window as unknown as { officer: unknown }).officer = makeOfficer({ memoryUpdate: update })
    render(<MemoryPanel />)
    await screen.findByText('Scourge is meta in WvW.')
  ...
  it('creates a fact via the Add fact form', async () => {
    ...
    ;(window as unknown as { officer: unknown }).officer = makeOfficer({ memoryCreate: create })
    render(<MemoryPanel />)
    await screen.findByText('Scourge is meta in WvW.')
