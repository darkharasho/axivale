 ...
 * profession or elite-spec name (case-insensitive), tolerating "core"
 * qualifiers and plurals (see classIconKeys). Regenerate the data module
 * with `node scripts/gen-class-icons-svg.cjs`.
 */
export function classIconSvg(name: string | null | undefined): string | null {
  return lookupClassIcon(CLASS_ICON_SVG_DATA, name)
}
